...
def initialize_clients():
    ...
    global llm, memgraph_graph, embedding_model # Add embedding_model to global
...
def query_engine(request):
    """
    Answers a global sensemaking query using the community summaries.
    """
    try:
        initialize_clients()
        ...
        # 2. Fetch community summaries and embeddings from Memgraph
        # Ensure that the consolidator is storing embeddings in Memgraph
        communities_data = memgraph_graph.query("MATCH (c:Community) WHERE c.embedding IS NOT NULL RETURN c.id AS community_id, c.summary AS summary, c.embedding AS embedding")
